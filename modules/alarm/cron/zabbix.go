...
func NewSender(host string) *ZabbixSender {
	s := &ZabbixSender{Host: host}
	return s
}

// Method Sender class, return zabbix header.
func (s *ZabbixSender) getHeader() []byte {
	return []byte("ZBXD\x01")
}

// Method Sender class, resolve uri by name:port.
func (s *ZabbixSender) getTCPAddr() (iaddr *net.TCPAddr, err error) {
	// format: hostname:port

	// Resolve hostname:port to ip:port
	iaddr, err = net.ResolveTCPAddr("tcp", s.Host)
	if err != nil {
...
func (s *ZabbixSender) connect() (conn *net.TCPConn, err error) {
...
func (s *ZabbixSender) read(conn *net.TCPConn) (res []byte, err error) {
	res = make([]byte, 1024)
...
func (s *ZabbixSender) Send(packet *Packet) (res []byte, err error) {
	conn, err := s.connect()
	if err != nil {
		return
	}
